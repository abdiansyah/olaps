...
class Quality_control extends MX_Controller {
	...
	public function index() {	   
		$this->page->view('quality_change_index', array (			
            ...
            'date(format)e_approved_superior'      => $this->input->post('date_approved_superior'),
 ...
 private function form($action = '', $id = ''){
		if ($this->agent->referrer() == '') redirect(site_url('quality_control'));		
		$title = '';
...
//            die();
    ...
    if(isset($_POST['save_validation_document'])){
        $per_num        = $this->input->post('personnel_number');
        $name_file      = $this->input->post('name_file');        
        $code_file      = $this->input->post('code_file');
        ...
        $sess_data_personnel    = $this->model_quality_control->get_data_row_personnel_by($per_num);
        $personnel_number       = $sess_data_personnel['personnel_number'];
        $name_personnel         = $sess_data_personnel['name'];        
        $unit                   = $sess_data_personnel['departement'];
        $presenttitle           = $sess_data_personnel['presenttitle'];                
        $url                    = $_SERVER['HTTP_REFERER'];                                                             
        $cek_data_document = $this->model_quality_control->cek_data_document($per_num);                   
        $this->load->library('email', $config);
        $this->email->set_newline("\r\n");
        $this->email->from($email);
        $this->email->to($personnel_number);
        $this->email->subject('APPLY LICENSE');
        $pesan = '<!DOCTYPE html PUBLIC "-W3CDTD XHTML 1.0 StrictEN"
                 "http:www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd"><html>
