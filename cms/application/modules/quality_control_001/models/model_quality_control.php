...
class model_quality_control extends CI_Model
{
    ...
    private function _get_query()
    {
        ...
        $this->db->select("(CONVERT(varchar(10), CONVERT(datetime, t_apply_license.date_request,120),105)) AS date_request, t_apply_license.request_number, TSH.name, TSH.personnel_number, 
                        ...
                        (CASE WHEN t_apply_license.take_authorization = '1' THEN 'Take Authorization'
    ...
    $query = "SELECT (tfr.name_file) AS name_file_ftp, tfr.personnel_number_fk, mglrg.category_continous, tfr.code_file, (maarg.name_t) AS name_file, tfr.id_auth_license_fk, tfr.id_auth_type_fk, tfr.date_training, tfr.expiration_date, tfr.status_valid, tfr.reason FROM t_file_requirement AS tfr
            LEFT JOIN m_auth_additional_req_general AS maarg ON tfr.code_file = maarg.code_t
            LEFT JOIN m_group_license_req_general AS mglrg ON mglrg.id_auth_additional_req_general_fk = maarg.id
            WHERE tfr.personnel_number_fk = '$personnel_number'";
