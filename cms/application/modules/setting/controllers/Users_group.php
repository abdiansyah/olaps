...
class Users_group extends MX_Controller {
	...
	public function insert(){		
		...
		if($menu != ''){
			foreach ($menu as $p){
				if($p != '0'){
					$this->db->insert('menu_akses', array('id_menu_fk' => $p, 'id_group_fk' => $id_users_group));
				}			
			}
		}
	...
	public function update($id){
		...
		$menu = $this->input->post('id_menu');
		if($menu != ''){
			$this->db->delete('menu_akses',array('id_group_fk' => $id));
			foreach ($menu as $p) {
				if($p != '0')
				{					
					$this->db->insert('menu_akses', array('id_menu_fk' => $p,'id_group_fk' => $id));
				}			
			}
		}	
    ...
    public function delete($id){				
		$this->db->delete('m_group', array('id_group' => $id));		
		redirect($this->page->base_url());
	}
