...
?>                                     
    ...
    if (is_array(@$data_file_general_document_quality) || is_object(@$data_file_general_document_quality)) {
        ...
        <tr>                                                                                               
        <td width="15%"><label class="label_general_document_quality"><?php
            echo @$row->name_t;?></label><input type="hidden" class="no_row_general_document_quality" value="<?php
            echo $i;?>"/></td>                                                                                                                                                                             
        <td width="10%"><input type="text" class="form-control expiration_date_general_quality" value="<?php
            if (@$row->expiration_date != '1900-01-01') {
                echo @$row->expiration_date;
            }?>"/></td>
        <td width="5%"><input type="hidden" id="code_req_general_document_quality_<?php
            echo @$row->code_t;?>" name="code_req_general_document_quality[]" value="<?php
            echo @$row->code_t;?>" />
        <input type="file" id="file_req_general_document_quality_<?php
            echo @$row->code_t;?>" class="file_req_general_document_quality" name="file_req_general_document_quality[]"/>
        </td>                                                                                                 
        <td width="10%">
        <div class="progressbox"><div id="progressbar_document_general_quality_<?php
            echo $i;?>" class="progress"></div></div>                                                                                                             
        </td> 
        <form action="<?php
        ...
        <td width="10%"><textarea disabled><?php
            echo @$row->reason;?></textarea></td>              
        </tr>
        <?php
            $i++;
        endforeach;
...
?>
    ...
    if (is_array(@$data_file_spec_document_quality) || is_object(@$data_file_spec_document_quality)) {
        ...
        <td width="15%"><label class="label_spec_document_quality"><?php
            ...
            echo $i;?>"/></td>                                                                                                                                                                             
        <td width="10%"><input type="text" class="form-control expiration_date_spec_quality" value="<?php
            if (@$row->expiration_date_spec_quality != '1900-01-01') {
                echo @$row->expiration_date_spec_quality;
            }?>"/></td>
...
?>
        ...
        <div class="col-md-12">
        <br />
        <button type="submit" class="btn btn-info pull-right btn-sm" name="save_validation_document_tqd"><strong>Save</strong></button>
        </form> 
        <a class="btn btn-info pull-right btn-sm" href="<?php echo site_url('quality_control/index');?>">BACK</a>        
             </div> 
        </div>
