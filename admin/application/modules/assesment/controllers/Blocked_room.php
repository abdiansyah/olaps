...
class Blocked_room extends MX_Controller {
    ...
    public function ajax_blocked_room(){		    
		...
		foreach ($list as $grid) {			
            ...
            $row[] = $grid->total_days;
            $row[] = $grid->reason;	
            $row[] = '<a href="'.site_url('/assesment/blocked_room/edit/'.$grid->id).'" title="Edit Data"><button class="btn btn-info btn-sm">Update</button></a> &nbsp; &nbsp; <a href="'.site_url('/assesment/blocked_room/delete/'.$grid->id).'" title="Edit Data"><button class="btn btn-warning btn-sm">Delete</button></a>';		                                                                                						          																												
			$data[] = $row;            
		}
