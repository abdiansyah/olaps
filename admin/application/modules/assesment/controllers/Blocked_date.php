...
class Blocked_date extends MX_Controller {
    ...
    public function ajax_blocked_date(){		    
		...
		foreach ($list as $grid) {			
            ...
            $row[] = $grid->total_days;
            $row[] = $grid->reason;	
            $row[] = '<a href="'.site_url('/assesment/blocked_date/edit/'.$grid->id).'" title="Edit Data"><button class="btn btn-info btn-sm">Update</button></a> &nbsp; &nbsp; <a href="'.site_url('/assesment/blocked_date/delete/'.$grid->id).'" title="Edit Data"><button class="btn btn-warning btn-sm">Delete</button></a>';		                                                                                						          																												
			$data[] = $row;            
		}
