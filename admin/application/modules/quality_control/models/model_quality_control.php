...
class model_quality_control extends CI_Model
{
   ...
   public function get_quality_control_finish()
    ...
    public function by_request_number($id)
    {
        ...
        return $datasrc->result();
    }

    public function get_assesment_history($id){
        $datasrc = $this->db->query("SELECT 
    ...
    public function get_data_requirement($request_number='',$personnel_number='', $id_authorization='', $id_type=''){
        if(!empty($id_authorization) && !empty($id_type)){
            $query = "EXEC dbo.requirement_applicant_by @request_number = '$request_number', @personnel_number = '$personnel_number', @id_authorization = '$id_authorization',@id_type = '$id_type' ";
        } else {
            $query = "EXEC dbo.requirement_applicant @request_number = '$request_number', @personnel_number = '$personnel_number'";
        }        
    ...
    public function get_spec_document_quality($request_number){
        $query = " SELECT tfr.personnel_number_fk, tfr.code_file, tfr.date_training, tfr.expiration_date, tfr.no_upload, tfr.name_file, 
            tfr.status_valid, tfr.reason, (maars.name_t) AS name_file, (tfr.name_file) AS name_file_ftp FROM t_apply_license_dtl AS tald
            JOIN m_group_scope_category AS mgsc 
            ON tald.id_auth_license_fk = mgsc.id_auth_license_fk
            ...
            JOIN m_auth_additional_req_spec AS maars ON mgsc.id_auth_additional_req_spec_fk = maars.id
            JOIN t_file_requirement AS tfr ON maars.code_t = tfr.code_file
            AND tfr.no_upload = (SELECT MAX(no_upload) AS no_upload FROM t_file_requirement WHERE t_file_requirement.code_file = tfr.code_file)
            WHERE tald.request_number_fk = '$request_number' AND  mgsc.category_admin = 'Quality'";        
        return $this->db->query($query)->result();
    } 
    ...
    public function get_validation_status($request_number=''){
        $query = "SELECT (CASE 
                WHEN tal.take_authorization = '1' THEN 'Take Authorization'
                WHEN tal.referral_authorization = '1' THEN 'Referral Authorization'                        
                WHEN tal.status_issue_authorization = '2' THEN 'Issue Authorization Finished'
                WHEN tal.status_assesment = '2' THEN 'Assesment Process Closed' 
                WHEN tal.status_approved_quality = '1' THEN 'Data Validated' 
                WHEN tal.status_approved_quality = '2' THEN 'Data Rejected'
                WHEN tal.status_approved_superior = '2' THEN 'Rejected Superior'
                WHEN tal.status_approved_superior = '1' THEN 'Approved Superior' 
                WHEN tal.status_submit = '1' THEN 'Data Submited'
                END) as current_status
                FROM t_apply_license AS tal
                WHERE tal.request_number = '$request_number'";
    ...
    public function get_type($id_auth){
        $query ="SELECT b.name_t, b.id FROM m_group_type_license a
                LEFT JOIN m_auth_type b ON a.id_auth_type_fk = b.id
                WHERE a.id_auth_license_fk = '$id_auth'";                     
    return $this->db->query($query)->result();
    }
