...
class Requirement extends MX_Controller {
	...
	private function form($action = 'insert_specific', $id = ''){
	...
	public function delete_specific($id){
		if ($this->agent->referrer() == '') redirect(site_url('license/requirement'));
	...
	public function option_type(){
		...
		$m_type = $this->db->query("SELECT id , name_t AS name_type FROM m_auth_type WHERE id = '$id_license' ORDER BY name_type");
    ...
    function get_license($param)
    {
        ...
        $this->load->view('license/tab_authorization/view_tab_type', $data);
        return true;
    }
    ...
    {
        $data['auth_spec'] = $this->model_requirement->get_type($param);
        $this->load->view('license/tab_authorization/view_tab_spec', $data);
        return true;
    }
    // -- Function Name : get_spec
    // -- Params : $p_id_spect, $p_id_license, $p_id_type
    // -- Purpose : 
    function get_spec($id_license, $id_type, $id_spect)
    {       
        $data['auth_category'] = $this->model_requirement->get_spec($id_license, $id_type, $id_spect);         
        $this->load->view('license/tab_authorization/view_tab_category', $data);
        return true;
    }
    // -- Function Name : get_category
    // -- Params : $p_category, $p_spect, $p_type, $p_license
    // -- Purpose : 
    function get_category($license, $type, $spect, $category)
    {
        $data['auth_scope'] = $this->model_requirement->get_category($license, $type, $spect, $category);
        $this->load->view('license/tab_authorization/view_tab_scope', $data);
        return true;
    }
