...
class Page {
	...
	public function __construct() {
		...
		if ($this->_CI->uri->segment(2) == 'pdf' OR $this->_CI->uri->segment(3) == 'pdf') {
			
		}
		else if($cek == '' || $cek == 'site') {
			$cek2 = $this->_CI->uri->segment(2);
			if($cek2 == 'logout') {
				
			}
			elseif($this->_CI->session->userdata('users_quality') != '') {
				redirect(site_url('/dashboard/'));
			}
		}
		else {
			if($this->_CI->session->userdata('users_quality') == '') {
				redirect(base_url());
			}
			else{
				$uri = $this->_CI->uri->uri_string();
				//$id_grup_users = $this->_CI->session->userdata('users_quality')->id_grup_users;
				if($uri != 'dashboard')	{
