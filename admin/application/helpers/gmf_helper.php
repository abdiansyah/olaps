...
function kode_tbl_req_spec(){
	$CI 	=& get_instance();	
	$query = "SELECT 
		MAX(code_t) AS id_req_spec
		FROM m_auth_additional_req_spec
	";
	$row = $CI->db->query($query)->row_array();
	$id = $row['id_req_spec'];
	$max_id = substr($id,4);
	...
	$initial_code = "ARS_";
	if($plus<10){                
		$kode = $initial_code."000".$plus;
	}else if($plus<99){                
		$kode = $initial_code."00".$plus;
	}else if($plus<999){                
		$kode = $initial_code."0".$plus;
	}else if($plus<9999){                
		$kode = $initial_code.$plus;
	}    	
	return $kode;
}

function kode_tbl_req_general(){
	$CI 	=& get_instance();	
	$query = "SELECT 
		MAX(code_t) AS id_req_general
		FROM m_auth_additional_req_general
	";
	$row = $CI->db->query($query)->row_array();
	$id = $row['id_req_general'];
	$max_id = substr($id,4);
	$plus = $max_id+1;
	$initial_code = "ARG_";
	if($plus<10){                
